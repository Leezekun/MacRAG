...
choices = [
    ...
    "vicuna-v1.5-7b-16k", "Llama3-70b-8k", "Llama2-13b-chat-longlora",
    ...
    "qwen1.5-7b-chat-32k", "qwen2.5-7b-instruct", "qwen2.5-14b-instruct",
    "llama3-8b-instruct-8k", "llama3.1-8b-instruct", "mistral-7b-instruct-v0.1", "mistral-7b-instruct-v0.3", "gemma-2-9b-it"
]
...
#            import pdb;pdb.set_trace()
            ...
            encodeds = tokenizer.apply_chat_template(messages, return_tensors="pt", return_dict=True).to(model.device)
            ...
            outputs = model.generate(**encodeds, max_new_tokens=max_new_tokens)
            response = tokenizer.decode(outputs[0][encodeds.input_ids.shape[-1]:], skip_special_tokens=True)
            return response, prompt_len
...
def search_q(question):
    ...
    if args.fil:
        ...
        if not fil_pred:
            ...
            fil_pred, doc_len = search_cache_and_predict(fil_pred, f'{log_path}/fil_pred.json', 'fil_pred', question, model_name, model, tokenizer, lambda: create_prompt(''.join(filter_output), question), maxlen, doc_len, 'Fil')
    # Extractor
    if args.ext:
        ...
        if not ext_pred:
            ...
            ext_pred, doc_len= search_cache_and_predict(ext_pred, f'{log_path}/ext_pred.json', 'ext_pred', question, model_name, model, tokenizer, lambda: create_prompt(''.join(rerank + extractor_output), question), maxlen, doc_len, 'Ext')
    # Extractor & Filter
    if args.ext_fil:
        ...
        if not ext_fil_pred:
            ...
            if not extractor_output:
                extractor_output = extractor(question, rerank, match_id)
            ext_fil_pred,doc_len = search_cache_and_predict(ext_fil_pred, f'{log_path}/ext_fil_pred.json', 'ext_fil_pred', question, model_name, model, tokenizer, lambda: create_prompt(''.join(filter_output + extractor_output), question), maxlen, doc_len, 'E&F')
    # RAG-Base
    if args.rb:
        rb_pred = load_cache(f'{log_path}/rb_pred.json', 'rb_pred', question, doc_len, 'R&B')
        if not rb_pred:
            rb_pred, doc_len = search_cache_and_predict(rb_pred, f'{log_path}/rb_pred.json', 'rb_pred', question, model_name, model, tokenizer, lambda: create_prompt(''.join(rerank), question), maxlen, doc_len, 'R&B')
    # RAG-Long
    if args.rl:
        rl_pred = load_cache(f'{log_path}/rl_pred.json', 'rl_pred', question, doc_len, 'R&L')
        if not rl_pred:
            rl_pred, doc_len = search_cache_and_predict(rl_pred, f'{log_path}/rl_pred.json', 'rl_pred', question, model_name, model, tokenizer, lambda: create_prompt(''.join(s2l_doc(rerank, match_id, maxlen)[0]), question), maxlen, doc_len, 'R&L')
...
if __name__ == '__main__':
    ...
    log_path = args.log_path or f'./log/{args.r_path.split("/")[-1]}/{args.dataset}/{args.model}/{args.lrag_model or "base"}'
